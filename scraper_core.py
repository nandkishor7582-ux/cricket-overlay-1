...
def parse(html, data):
    ...
    desc = re.sub(r'\s+', ' ', meta["content"] if meta and meta.get("content") else "")

    if not data["team1"]["name"]:
        # "Follow IND 185/3 (20.0) vs PAK 160/8"
        dm = re.search(
        ...
        if dm:
            ...
            # At minimum extract team names from "X vs Y" in title/desc
            vs_m = re.search(r'([A-Z][A-Za-z\s&]{1,20}?)\s+vs\.?\s+([A-Z][A-Za-z\s&]{1,20}?)(?:\s*[,|\-|–]|$)', full[:500])
            if vs_m:
                if not data["team1"]["name"]: data["team1"]["name"] = vs_m.group(1).strip()
                if not data["team2"]["name"]: data["team2"]["name"] = vs_m.group(2).strip()
